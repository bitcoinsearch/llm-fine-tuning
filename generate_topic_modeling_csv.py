...
from src.utils import preprocess_email, log_csv
    ...
    try:
        ...
        for dev_url in dev_urls:
            ...
            try:
                ...
                # if UPDATE_ES_SIMULTANEOUSLY set to True, it will update topics in the elasticsearch docs as we
                # generate them
                    ...
                    for idx, doc in enumerate(tqdm.tqdm(docs_list)):
                        ...
                        if not doc['_source'].get('primary_topics'):
                            ...
                            if doc_text:
                                ...
                                try:
                                    primary_kw, secondary_kw = apply_topic_modeling(text=doc_text,
                                                                                    topic_list=btc_topics_list)
                                    generated_count += 1
                ...
                log_csv(file_name="generate_topic_modeling_csv",
                        url=dev_url,
        ...
        for url, status in url_status.items():
            logger.info(
                f"URL: {url} - Generated Count: {status['generated_count']} - Updated Count: {status['updated_count']} - Error Occurred: {status['error_occurred']}.")
